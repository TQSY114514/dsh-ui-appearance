...
function decodeTex(data) {
  ...
  switch (parsed.format) {
    case TexFormat.RGBA8888: {
      ...
      decodedRgba = bytes.slice(0, width * height * 4);
      break;
    }
    case TexFormat.R8: {
      ...
      for (let i = 0; i < width * height; i++) {
        decodedRgba[i * 4] = bytes[i];
        decodedRgba[i * 4 + 1] = bytes[i];
        decodedRgba[i * 4 + 2] = bytes[i];
    ...
    case TexFormat.RG88: {
      ...
      decodedRgba = new Uint8Array(width * height * 4);
      for (let i = 0; i < width * height; i++) {
        decodedRgba[i * 4] = bytes[i * 2];
        decodedRgba[i * 4 + 1] = bytes[i * 2 + 1];
        decodedRgba[i * 4 + 2] = 0;
        decodedRgba[i * 4 + 3] = 255;
      }
      break;
    ...
    case TexFormat.DXT1: {
      ...
      decodedRgba = decodeDxt1(bytes, width, height);
      break;
    }
    case TexFormat.DXT3: {
      ...
      decodedRgba = decodeDxt3(bytes, width, height);
      break;
    }
    case TexFormat.DXT5: {
      const expected = Math.ceil(width / 4) * Math.ceil(height / 4) * 16;
      if (bytes.length < expected) throw new Error('tex: mipmap size mismatch for DXT5');
      decodedRgba = decodeDxt5(bytes, width, height);
      break;
    }
    default:
  ...
  // If the TEX container declared a smaller active image size than the texture allocation
  // (e.g. image is 1920x1080 inside a 2048x2048 POT D3D texture pitch), crop to valid image area!
  const targetW = (parsed.imageWidth > 0 && parsed.imageWidth < width) ? parsed.imageWidth : width;
  const targetH = (parsed.imageHeight > 0 && parsed.imageHeight < height) ? parsed.imageHeight : height;
  if (targetW < width || targetH < height) {
    const cropped = new Uint8Array(targetW * targetH * 4);
    for (let y = 0; y < targetH; y++) {
      cropped.set(decodedRgba.subarray((y * width) * 4, (y * width + targetW) * 4), y * targetW * 4);
    }
    return { kind: 'rgba', width: targetW, height: targetH, rgba: cropped };
  }
  return { kind: 'rgba', width, height, rgba: decodedRgba };
}
...
function tryCompositeSceneLayers(scene, access, label) {
  ...
  for (const obj of imageObjects) {
    ...
    const alpha = typeof obj.alpha === 'number' && Number.isFinite(obj.alpha)
      ...
      : 1;
    // Convert Wallpaper Engine Y-up coordinates (Y=0 at bottom) to canvas top-down coordinates:
    const cyTopDown = projection ? (projection.height - cy) : -cy;
    layers.push({ img, cx, cy: cyTopDown, lw: Math.round(lw), lh: Math.round(lh), alpha });
  }
  if (layers.length === 0) return null;
  if (layers.length === 1) {
    const L = layers[0];
    const needsCrop = (projection && (L.lw !== L.img.width || L.lh !== L.img.height))
      || (L.lw < L.img.width || L.lh < L.img.height);
...
function extractSceneMainImageVia(access, label) {
  ...
  for (const path of access.listTexPaths()) {
    try {
      ...
      if (mp4) {
        ...
        videoCandidates.push({ path, mp4, width: w, height: h, score });
      }
    } catch { /* ignore */ }
  }
  ...
  } catch { /* fall through to single-texture path */ }

  // Rank all textures by desktop wallpaper artwork score:
  // - Resolution & Area: 4K/8K gets highest priority
  // - Aspect ratio: desktop widescreen (1.2 ~ 2.6) gets heavy boost; vertical strips / tiny particles heavily penalized
  // - Positive keywords: background, bg, wallpaper, 壁纸, 背景, main, scene
  // - Negative keywords: particle, mask, effect, halo, flare, bar, bokeh, pole, character parts
  const ranked = [];
  for (const path of access.listTexPaths()) {
    let score = 0;
    try {
      ...
      if (info && !info.isVideoMp4) {
        ...
        if (aspect >= 1.2 && aspect <= 2.6) {
          score *= 4.0;
        } else if (aspect >= 0.85 && aspect < 1.2) {
          score *= 1.0;
        } else {
          score *= 0.01; // Vertical strips or extreme ratios
        }

        const l = path.toLowerCase();
        if (l.includes('bg') || l.includes('background') || l.includes('背景') || l.includes('壁纸') || l.includes('main') || l.includes('origin')) {
          score *= 5.0;
        }
        if (
          PATH_PENALTY_RE.test(path) ||
          l.includes('particle') ||
          l.includes('mask') ||
          l.includes('effect') ||
          l.includes('halo') ||
        ...
        if (l.includes('发') || l.includes('手') || l.includes('眼') || l.includes('耳') || l.includes('鼻') || l.includes('杆') || l.includes('刘海') || l.includes('垂发')) {
          score *= 0.05;
        }
      }
    } catch {
  ...
  // Fallback candidates from scene objects if ranking was empty
  if (candidates.length === 0) {
    for (const obj of scene.objects) {
      if (obj && typeof obj === 'object' && typeof obj.image === 'string') {
        candidates.push(...collectImageObjectTextures(obj, access.readJson));
      }
    }
  }
    ...
    try {
      ...
      if (mp4) {
        ...
        return {
          mime: 'video/mp4',
          bytes: mp4,
          width: info ? info.width : 3840,
          height: info ? info.height : 2160,
      ...
      // Raw RGBA — trim transparent bottom/right POT padding if present, then apply quality gate.
      const trimmed = trimTransparentPadding(decoded.rgba, decoded.width, decoded.height);
      const q = frameQuality(trimmed.width, trimmed.height, trimmed.rgba);
      ...
      return {
        mime: 'image/png',
        bytes: encodePng(trimmed.width, trimmed.height, trimmed.rgba),
        width: trimmed.width,
...
/**
 * Trim transparent bottom and right padding (e.g. from Power-Of-Two padded textures
 * where the actual artwork occupies only the top-left portion like 1920x1080 inside 2048x2048).
 */
function trimTransparentPadding(rgba, width, height) {
  let minX = width;
  let maxX = 0;
  let minY = height;
  let maxY = 0;
  ...
  if (!hasPixels) return { rgba, width, height };
  if (minX === 0 && minY === 0 && (maxX + 1 < width || maxY + 1 < height)) {
    const newW = maxX + 1;
    const newH = maxY + 1;
    if (newW >= 320 && newH >= 240) {
      const cropped = new Uint8Array(newW * newH * 4);
      for (let y = 0; y < newH; y++) {
        cropped.set(rgba.subarray((y * width) * 4, (y * width + newW) * 4), y * newW * 4);
      }
      return { rgba: cropped, width: newW, height: newH };
    }
  }
