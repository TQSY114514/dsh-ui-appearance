...
- **背景图片**:点击上传或拖拽上传(JPG / PNG / WebP 等),带预览、更换、删除;图片自动压缩(最长边 1920px)以便持久化
...
- 背景图片需配合「面板透明度」<100% 才会透出(界面默认不透明以保可读性);界面区有对应提示文案。
- 上传图片会被压缩(最长边 1920px、JPEG 质量 0.85)后以 data URL 存入设置文档;原图不会被保留。
