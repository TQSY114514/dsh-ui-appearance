...
- **安装或更新后没有看到设置面板？**
  1. 检查目标 profile：确认插件已安装到当前实际运行的 profile（DSH Desktop 默认 profile 一般为 `desktop`，Web 版通常为 `web`）。
  2. 源码安装排查：若是克隆源码直装，请先执行 `pnpm prepare` 构建生成 `lib/`，并重启 DSH 服务。
  3. 浏览器强刷：按 `Ctrl + F5`（Mac 为 `Cmd + Shift + R`）强制刷新以清除浏览器缓存的旧 bundle。
- **网络 URL 图片 / 视频无法加载？**
  部分外链图床或视频平台开启了防盗链或缺少 CORS 跨域头，导致浏览器安全策略拦截加载。建议将图片/视频下载到本地后拖拽上传，或使用支持 CORS 访问的直链。
- **与其他插件共存时样式发生冲突？**
  本插件完全通过官方 `--dsw-*` 语义 token 系统与非侵入式背景图层实现。若共存插件存在硬编码全局覆盖样式，可能会导致视觉异常，可尝试更新相关插件至最新版本或排查全局样式注入。
- **如何彻底重置所有外观设置？**
  可在面板中点击「默认」预设并删除壁纸/视频；或在浏览器开发者工具 Console 执行 `localStorage.removeItem('dsh-ui-appearance.settings')` 后刷新页面。卸载插件（`dsh plugin remove`）时也会自动注销所有样式并彻底恢复原生外观。
