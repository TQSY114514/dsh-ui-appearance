...
- **Q: 为什么部分文字或特定元素（如代码块、气泡）颜色未随主题改变？**
  - **代码高亮**：采用独立的 Shiki 语法高亮主题以保持 IDE 规范，不随界面主色变化。
  - **消息气泡**：DSH 宿主目前仅在用户消息上渲染气泡容器（AI 消息无气泡容器），插件已自动将用户气泡跟随主色调。
  - **局部文字**：部分未映射 `--dsw-*` 语义 token 的组件，后续版本会随官方扩展点演进持续拓展。
- **Q: 壁纸或视频背景无法显示 / 加载失败？**
