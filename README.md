...
3. 点预设快速换肤 → 取色器/HEX 微调每个颜色角色 → 上传或拖入背景图片 → 拖动不透明度/模糊滑块。
...
| 界面不透明度 | 表面 token 覆写为按模式烘焙的 `rgba()` 半透明值(角色色 → 深色翻转色 → 默认面色表) |
...
- 设置背景图片后主区域自动透出壁纸;卡片、侧边栏、输入框默认不透明,调低「面板不透明度」可让它们也透出
