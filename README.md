...
- **Q: 安装或更新后没有看到「个性化外观」设置项？**
  - **检查 Profile**：确认插件安装在当前运行的 profile（DSH Desktop 默认 profile 为 `desktop`，Web 版通常为 `web`；可运行 `dsh plugin list` 检查）。
  - **重启服务**：安装后需重启服务（如 `dsh web`）；克隆源码直装请确保先执行过 `pnpm prepare` 编译。
  - **清除缓存**：按 `Ctrl + F5`（Mac 为 `Cmd + Shift + R`）强制刷新浏览器，清除旧的 bundle 缓存。
- **Q: 修改外观后需要手动刷新或保存吗？**
  - 不需要。所有颜色、透明度、壁纸与氛围调整均为**实时响应、自动持久化**。若未即时生效，可尝试刷新页面。
- **Q: 我的个性化配置会持久保存吗？**
  - 会。设置保存在浏览器 `localStorage`，壁纸与视频原画质保存在 `IndexedDB`，刷新或重启均会保留。
  - **注意**：配置跟随浏览器本地，换浏览器或清除站点数据会重置；可通过「配色方案 $\rightarrow$ 导出」将配置备份为一段 JSON 文本。
- **Q: 卸载插件后会影响 DSH 原始界面吗？**
  - 完全不会。本插件基于官方扩展机制开发，零修改核心代码。执行 `dsh plugin remove dsh-ui-appearance` 卸载后会自动回收所有注入的样式表与图层，彻底恢复默认。
- **Q: 壁纸或视频背景无法显示 / 加载失败？**
...
- **Q: 为什么消息气泡（AI 消息）不能单独改颜色？**
  - 这是由于 DSH 宿主目前的渲染机制：DSH 只在用户消息上渲染独立气泡容器，AI 消息本身没有气泡包裹。插件会将用户气泡跟随主色调，受限于宿主结构无法单独拆分。
- **Q: 为什么代码高亮颜色没有跟随主题主色？**
  - 代码高亮使用的是独立 Shiki 语法高亮主题（保持 IDE 代码可读性惯例），不随 UI 基础主题色 token 变化。
- **Q: 与其他第三方 UI/主题插件共存冲突怎么办？**
  - 本插件严格通过 `--dsw-*` 语义 token 实现。若共存插件包含硬编码全局覆盖样式（如强制重写全局 background 或 color），可能会引发冲突，可先尝试暂时禁用其他外观插件排查。
- **Q: 如何分享或导入别人的配色？**
