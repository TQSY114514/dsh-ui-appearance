...
| 气泡角色 | harness 将所有消息气泡渲染为单一 token(`--dsw-specific-bubble`),「AI 消息气泡」优先映射该 token;「用户消息气泡」在 AI 未设置时同样映射(harness 的 `bubble-highlight` 无消费方,仅作保留) |
...
- 「用户消息气泡」与「AI 消息气泡」共用同一气泡底色(harness 渲染限制),两者都设置时 AI 消息气泡优先
